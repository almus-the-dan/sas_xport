...
use crate::sas::xport::async_xport_buffer::AsyncXportBuffer;
...
#[derive(Debug)]
pub struct AsyncXportDataset<R> {
    buffer: AsyncXportBuffer<R>,
    state: XportDatasetState,
}

impl<R> AsyncXportDataset<R> {
...
impl<R: AsyncBufRead + Unpin> AsyncXportDataset<R> {
    pub(crate) fn new(
        buffer: AsyncXportBuffer<R>,
    ...
    /// Will return Err if:
    /// * An I/O error occurs while trying to read the file
    pub async fn next_lazy_record(&mut self) -> Result<Option<LazyXportRecord<'_>>> {
        if self.state.complete() {
            return Ok(None);
        }
        let outcome = self.find_record().await?;
    ...
    async fn find_record(&mut self) -> Result<FindRecordOutcome> {
        if self.state.record_length() < HEADER_LENGTH {
            self.find_narrow_record().await
        } else {
            self.find_wide_record().await
        }
    }

    async fn find_narrow_record(&mut self) -> Result<FindRecordOutcome> {
        if let Some(outcome) = self.state.check_narrow_preloop() {
            return Ok(outcome);
        }
        loop {
            let outcome = self.find_wide_record().await?;
            if !matches!(outcome, FindRecordOutcome::Record) {
                return Ok(outcome);
            }
    ...
    async fn find_wide_record(&mut self) -> Result<FindRecordOutcome> {
    ...
    pub async fn next_dataset(mut self) -> Result<Option<Self>> {
        ...
        let schema = self.buffer.read_schema(metadata.file_version()).await?;
...
impl<R: AsyncBufRead + AsyncSeek + Unpin> AsyncXportDataset<R> {
    ...
    pub async fn seek(&mut self, index: u64) -> Result<()> {
        if index == self.state.record_number() as u64 {
            // We're already at the specified index. Avoid any unnecessary I/O
            // or discarding the current buffer.
            return Ok(());
        }
        // The record offset always points to the start of the observation data,
        // and the length of each record is a fixed size. We can determine the offset
        // of a specific record using a simple calculation.
        ...
        self.buffer.seek_from_start(position as u64).await?;
